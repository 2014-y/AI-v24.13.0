...
### Added
- **AI 内置客服错误诊断指南**：在 `renderer.js` 客服助手的 System Prompt 中新增常见报错排查：
  - **第 7 条 (Exec failed 诊断)**：详细解读了大模型 Agent 在后台调用 \`run_command\` 失败时（如中文系统下 systeminfo 英文关键字过滤失败，或语法解析 Bug）触发黄色警告横幅的成因，并指引用户安全忽略。
  - **第 8 条 (本地网关故障诊断)**：全面整理了本地 Node 进程及插件装载异常（EADDRINUSE 端口占用、.node-sandbox 沙箱环境损坏、MCP 工具链异常、大模型 API 转发层网络/密钥错误等）的自愈及解决步骤。

### Changed
- **渠道掉线排查升级**：将原有的“微信机器人断连排查”扩展为了覆盖微信、QQ、飞书、Slack、WhatsApp、Matrix 等全渠道机器人的通用连接排查。细化了注入机制、API参数校验、境内外代理设置差异，以及防睡眠策略，使客服小助手的网络和代理指导更具普适性。
